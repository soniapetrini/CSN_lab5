...
merge_communities <- function(graphs, difficulty = NULL) {
  ...
  for (i in 1:length(graphs)) {
    ...
    nodes[i+1] <- vcount(g)
    # rename vertices
    labels <- if (i==1) {seq(nodes[i],nodes[i+1])} else {seq(nodes[2],nodes[2]+nodes[2+1]-1)}
...
graphs <- c("Tutte","Coxeter","Thomassen","Meredith")#,"Folkman","Herschel","Icosahedral","Cubical")
syn_graph <- merge_communities(graphs, difficulty = 10)
plot(syn_graph)
```
...
plot(macaque_net,vertex.color=membership(communities))
```
