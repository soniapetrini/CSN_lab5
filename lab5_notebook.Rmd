...
```{r}
vertex_attr(karate_net) <- list(name=V(karate_net))
communities <- edge.betweenness.community(karate_net)
...
# Conductance new
  ...
  i <- 1
  for (c in clusters) {
    members <- memberships[memberships==1]
    nodes <- names(members)
    subgraph <- induced_subgraph(karate_net,nodes,impl = "auto")
