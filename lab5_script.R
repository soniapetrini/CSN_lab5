...
count_occurrencies <- function(x,targets) {
  occurrencies <- c()
  for (t in targets) {
    y <- length(which(x==t))
    occurrencies <- append(occurrencies,y)
  }
  names(occurrencies) <- targets
  return(occurrencies)
}
...
get_conductance <- function(graph,communities) {
  ...
  for (edge_index in 1:length(E(graph))) {
    ...
    if (memberships[edge_nodes[1]] != memberships[edge_nodes[2]]) {
      fc_list[i_2] <- memberships[edge_nodes[1]]
      fc_list[i_2+1] <- memberships[edge_nodes[2]]
      i_2 <- i + 2
    }
    
    else {mc_list[i] <- memberships[edge_nodes[1]]
    i <- i + 1}
  }
  
  fc <- count_occurrencies(fc_list,unique(memberships))
  mc <- count_occurrencies(mc_list,unique(memberships))
  
  conductances <- fc/(2*mc+fc)
  weights <- table(memberships)/n
